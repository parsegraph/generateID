...
app.get("/", async (req, res) => {
  ...
  write(`<html>`);
  write(`<head>`);
  write(`<title>generateid</title>`);
  write(`</head>`);
  write(`<body>`);
  write(
    `<h1>generateid <a href='/coverage'>Coverage</a> <a href='/docs'>Docs</a></h1>`
  );
  write(
    `<p>This library is available as JavaScript UMD module: <a href='/parsegraph-generateid.js'>parsegraph-generateid.js</a></p>`
  );
...
app.listen(port, () => {
  console.log(`See generateid build information at http://localhost:${port}`);
});
